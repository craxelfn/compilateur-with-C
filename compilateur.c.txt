			...
			if(strcmp(temp->Tab,"(\0")==0){
				...
				if(verifparenthese == 1){
					...
					Analyse_syntaxique(analyse_si,s,nlig);
						...
						fprintf(s,"Erreur syntaxique numero %d dans la ligne numero %d, 'alors' est attendue apres l'instruction \"si\"'. mot errone:'%s'\n",k,nlig,temp->Tab);
						k++;	
						}
						else{
							while(temp->next->next !=NULL){
...
int main() {
    ...
    char Tab_Ligne[10]; int i=0; //tableau des lignes avec son compteur i
	...
	//}
    free(debut);
    printf("si %d finsi %d",compteur_si,compteur_finsi);
    return 0;
}
